...
Use the `block_designs` attribute on `vivado_synthesize` or
`vivado_create_project` to fold this BD into a synth project; the
synth template's existing BD loop will pick it up and run
