...
Stages a directory of files containing a Vivado-packaged IP (a tree rooted at
one or more `component.xml` files) into a TreeArtifact and exposes it via
`VivadoIPBlockInfo` so consumers pick it up on `ip_repo_paths`. Use this
for third-party / vendor-supplied IPs. For Xilinx-catalog IPs configured
via a `create_ip` TCL use `vivado_xci`; for HDL packaged as a new IP use
`vivado_create_ip`.
"""
