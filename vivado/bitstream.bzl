...
vivado_write_bitstream = rule(
    ...
    attrs = {
        ...
        "post_hooks": attr.label_list(
            doc = "TCL files sourced after a successful `write_bitstream`, in list order.",
            allow_files = [".tcl", ".xdc", ".sdc"],
            default = [],
        ),
        "pre_hooks": attr.label_list(
            doc = "TCL files sourced on the opened checkpoint before `write_bitstream`.",
            allow_files = [".tcl", ".xdc", ".sdc"],
            default = [],
        ),
        "threads": attr.int(
...
def _vivado_write_device_image_impl(ctx):
    ...
    # `xsa` is always present (empty depset when `with_xsa = False`) so
    # consumers don't have to guard the access.
    output_groups = {
...
vivado_write_device_image = rule(
    ...
    attrs = {
        ...
        "post_hooks": attr.label_list(
            doc = "TCL files sourced after a successful `write_device_image`, in list order.",
            allow_files = [".tcl", ".xdc", ".sdc"],
            default = [],
        ),
        "pre_hooks": attr.label_list(
            doc = "TCL files sourced on the opened checkpoint before `write_device_image`.",
            allow_files = [".tcl", ".xdc", ".sdc"],
            default = [],
        ),
        "threads": attr.int(
